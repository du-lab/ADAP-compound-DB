...
//            }
    ...
    enum ExportField {
        ...
        QUERY_FILE_ID("File Index", null, r -> r.getQueryFileIndex() > 0 ? Integer.toString(r.getQueryFileIndex() + 1) : null),
        QUERY_SPECTRUM_ID("Numerical Signal ID assigned by ADAP-KDB", null, r -> r.getQuerySpectrumIndex() > 0 ? Integer.toString(r.getQuerySpectrumIndex() + 1) : null),
        MATCH_INDEX("Match #", null, r -> r.getMatchIndex() > 0 ? Integer.toString(r.getMatchIndex() + 1) : null),
        ...
        ISOTOPIC_SIMILARITY("Isotopic Similarity", ExportCategory.DIFFERENCE,
                r -> r.getIsotopicSimilarity() > 0.0 ? String.format("%.0f", 1000 * r.getIsotopicSimilarity()) : null),
        SCORE("Fragmentation Score by matching with Experimental spectra", ExportCategory.DIFFERENCE,
                r -> r.getScore() > 0.0 ? Long.toString(r.getNISTScore()) : null),
        ...
        KEGG_ID("Kegg ID", ExportCategory.MATCHED, SearchResultDTO::getKeggId),
        REFMET_ID("RefMet ID", ExportCategory.MATCHED, SearchResultDTO::getRefMetId),
